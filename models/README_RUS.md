...
### Пример:
Попробуем изменить трейл `trail_red_long`. В файле `models/effects.fxp` ищем строку `trail_red_long` (она будет одна на весь файл). Далее ищем ближайшую запись `FX_INFO_COLOURBRIGHT_DATA` под `trail_red_long`. После чего заменяем нужные значения:
...
сохраняем файл и перезаходим в игру
